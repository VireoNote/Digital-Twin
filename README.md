...
### Step 4: The Probability Matrix & Final Directive
The L3 Forecasting Center synthesizes the 5-dimensional data into a strict **Probability Matrix** instead of single-direction predictions. The output must include:
...
3. **Invalidation Nodes**: Explicitly defines the physical boundaries (e.g., "If DXY breaks 105") that would immediately invalidate the current base case.
4. **Brier Score Tracking**: Evaluates the accuracy of the previous forecast to suppress LLM hallucination and overconfidence.
...
| **Polymarket (Gamma Events API)** | Macro Narrative & Political Pricing | Implemented (Public Endpoint v2.2) | None (No Key Required) |
...
*   📡 **多普勒雷达专员 (`doppler-radar`) v2.2**：利用 Polymarket `/events` 接口，监控交易量超百万美金的突发政策、政治大选与宏观事件的叙事动量。
...
### 步骤 4：概率矩阵与终极指令输出
L3 预报中心在输出最终报告时，必须将 5 维数据融合为极具实战意义的研判体系：
1. **概率矩阵 (Probability Matrix)**：输出基准情景 (Base Case)、尾部风险 (Tail Risk) 与上行惊喜的发生概率。
2. **凯利公式仓位映射 (Kelly Criterion)**：根据估算胜率和赔率，建议绝对的最大资金敞口。
3. **失效节点 (Invalidation Nodes)**：明确定出打破当前概率预期的“物理红线”（如 DXY 突破某数值）。
4. **布莱尔分数追踪 (Brier Score)**：强制回测上期预报准确度，对抗 AI 过度自信的幻觉。
5. **数字分身终极操作建议**：用最具指令性的语言，向现货、多头、空头三种仓位下达干脆的交易动作指令。
...
| **Polymarket (Gamma Events API)** | 宏观叙事与预期定价 | 已实现 (公共接口 v2.2) | 无需 API Key，直接可用 |
